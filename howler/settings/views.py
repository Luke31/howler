...
from wally.elastic.index import IndexMail, IndexIrc
...
def synonym_import(request):
    ...
    # Update email index
    indexmail = IndexMail(es, es_index_prefix=djsettings.ES_SUPPORTED_INDEX_PREFIX['email'], es_type_name=djsettings.ES_SUPPORTED_TYPE_NAMES['email'],
                  user_dictionary_file=djsettings.JA_USER_DICT)
    indexmail.add_mapping_to_index_multi(delete_old_indices=False, kuromoji_synonyms=kuromoji_synonyms)  # Update index

    # Update irc index
    indexirc = IndexIrc(es, es_index_prefix=djsettings.ES_SUPPORTED_INDEX_PREFIX['irc'], es_type_name=djsettings.ES_SUPPORTED_TYPE_NAMES['irc'])
    indexirc.add_mapping_to_index('', djsettings.SUPPORTED_LANG_CODES_ANALYZERS['ja'], delete_old_index=False,
